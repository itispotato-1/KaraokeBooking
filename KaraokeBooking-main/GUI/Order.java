...
public class Order extends JDialog {
    ...
    private ProductSystem productCatalog;
    ...
    private void initComponents() {
        ...
        if (product.getProductId().charAt(0) == 'F') {
            for (toppings toppings : mainFrame.getProductSystem().getAllTopFood()) {
                toppinglist.add(toppings);
            }
        } else if (product.getProductId().charAt(0) == 'D') {
            for (toppings toppings : mainFrame.getProductSystem().getAllTopDrink()) {
                toppinglist.add(toppings);
        ...
        if (product.getProductId().charAt(0) == 'F') {
            for (int i = 0; i < checkBox.length; i++) {
                if (checkBox[i].isSelected()) {
                    for (toppings topping : mainFrame.getProductSystem().getAllTopFood()) {
            ...
            for (int i = 0; i < radioButton.length; i++) {
                if (radioButton[i].isSelected()) {
                    for (toppings topping : mainFrame.getProductSystem().getAllTopDrink()) {
    ...
    protected void jButtonConfirmActionPerformed(ActionEvent e) throws Exception {
        ...
        if (mainFrame.getUser().getMoney() - Double.parseDouble(textAreaCost.getText()) < 0) {
            ...
            mainFrame.getSystemMoney().setProfit(sumCost);
            mainFrame.getProductSystem().addOrder(USER_ID, product, stringTopping, sumCost, (int) spinnerAmount.getValue());// เพิ่มOrder
        }
    }
