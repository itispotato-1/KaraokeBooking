...
public class MyOrder extends JPanel {
        ...
        private void initComponents() {
                ...
                // -------------------------------- Panel อาหารและน้ำ -----------------
                JPanel jPanelFoodAndDrink = new JPanel();
                jPanelFoodAndDrink.setLayout(new BoxLayout(jPanelFoodAndDrink, BoxLayout.Y_AXIS));
                jPanelFoodAndDrink.setBackground(new Color(250, 248, 227));
                jPanelFoodAndDrink.setMaximumSize(new Dimension(400, 403));

                JPanel jPanelFoodAndDrinkTop = new JPanel(new FlowLayout(FlowLayout.CENTER));
                jPanelFoodAndDrinkTop.setBackground(Color.WHITE);
                jPanelFoodAndDrinkTop.setOpaque(false);
                jPanelFoodAndDrinkTop.setPreferredSize(new Dimension(350, 45));
                jPanelFoodAndDrinkTop.setMaximumSize(new Dimension(350, 45));
                jPanelFoodAndDrinkTop.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 5));
                ...
                jPanelFoodAndDrinkTop.add(jLabelOnlist, BorderLayout.CENTER);

                JPanel jPanelFoodAndDrinkCenter = new JPanel(new FlowLayout(FlowLayout.CENTER, 5, 5));
                jPanelFoodAndDrinkCenter.setBackground(Color.WHITE);
                jPanelFoodAndDrinkCenter.setOpaque(false);
                jPanelFoodAndDrinkCenter.setMaximumSize(new Dimension(380, 320));
                ...
                for (Component comp : panelFoodAndDrinkIn.getComponents()) {
                        ...
                        maxHeight = Math.max(maxHeight, bounds.y + bounds.height);//เทียบMax กับ ตำแหน่ง Y + ความสูง
                }
                ...
                jPanelFoodAndDrinkCenter.add(jScrollPane1);

                jPanelFoodAndDrink.add(jPanelFoodAndDrinkTop);
                jPanelFoodAndDrink.add(jPanelFoodAndDrinkCenter);
                ...
                panelSecound.add(jPanelFoodAndDrink);
        ...
        private void loadMyOrder() {
                ...
                int x = 20, y = 10, i = 0;
                for (String Order : mainframe.getProductSystem().getOrder(mainframe.getUserId())) {
                        ...
                        buttonCancel1[i].addActionListener(new ActionListener() {
                                @Override
                                public void actionPerformed(ActionEvent e) {
                                        int confirm = JOptionPane.showConfirmDialog(MyOrder.this,
                                                        "Do you want to remove x1 " + tempSplitNameOrder[0] + "?",
                                        ...
                                        if (confirm == JOptionPane.YES_OPTION) {
                                                ...
                                                String[] tempStr = mainframe.getService().getValueUserList(//
                                                                mainframe.getUser().getUsername(), //
                                                                mainframe.getUser().getPassword());//
                                                mainframe.getUser().setMoney(Double.parseDouble(tempStr[4]));// ตั้งค่าเงินของผู้ใช้
                                                mainframe.getUser().setMoney(
                                                                mainframe.getUser().getMoney()
                                                                                + ((PRICE * AMOUNT) * 0.70));
                                                mainframe.getService().setMoneyUserInUserList(mainframe.getUser());//
                                                mainframe.getSystemMoney().setProfit((-((PRICE * AMOUNT) * 0.70)));
                                                reGUI();
                                        }
                                }
                        });
                        ...
                        buttonCancelAll[i].addActionListener(new ActionListener() {
                                ...
                                public void actionPerformed(ActionEvent e) {
                                        int confirm = JOptionPane.showConfirmDialog(MyOrder.this,
                                                        "Do you want to removeAll " + tempSplitNameOrder[0] + "?",
                                                        "Confirmation", JOptionPane.YES_NO_OPTION);
                                        ...
                                        if (confirm == JOptionPane.YES_OPTION) {
                                                ...
                                                mainframe.getProductSystem().removeOrder(ID, PRODUCT,
                                                                AMOUNT);
                                                mainframe.getUser().setMoney(
                                                                mainframe.getUser().getMoney() + (PRICE * 0.70));
                                                mainframe.getService().setMoneyUserInUserList(mainframe.getUser());//
                                                reGUI();
                                        }
                                }
